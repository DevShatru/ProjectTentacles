@echo off
::
:: Build script for "10 Minutes to Breathe"
::
:: Copyright (C)2021 Scott Henshaw, All rights reserved
:: USAGE:
::   AutoBuild [--nodebug] [--nopull] [--release:no!network!only]
::
...
::
:: set COHORT="GD66PG22"
 set PROJECT="ProjectTentacle"

:: GIT:  Edit the line below to set your git repo if you use the clone option
:: edit your URL
set GIT_REPO="https://github.com/DevShatru/ProjectTentacles.git"
...
:netrelease
set RELEASE=%USERPROFILE%\Desktop\ProjectTentacle
...
set PROJ_PATH=%SRC%\\%PROJECT%
:: Build the code with UNREAL, uncomment the lines below
%UCC% %PROJECT% -ModuleWithSuffix %PROJECT% 5202 Win64 Development -editorrecompile -canskiplink %PROJ_PATH%.uproject >>%LOGFILE%

@echo:
@echo Starting %UNREAL_PRE% at %time:~0,8%
call %UNREAL_PRE% BuildCookRun -project=%PROJ_PATH%.uproject -noP4 -platform=Win64 -clientconfig=Shipping -serverconfig=Shipping -cook -allmaps -build -stage -pak -archive -archivedirectory=%DEST% >>%LOGFILE%


:: Build the code with Unity, comment out the unreal build and uncomment this
:: %UNITY% -projectPath %PROJ_PATH% -quit -batchmode -buildWindows64Player %PROJ_PATH%\Build.exe -logFile
